...
/* tdoom: raised from the upstream 1024.
 *
 * writeBytes() splits a frame into chunks of this many pixels and busy-waits on
 * each one via spi_device_polling_start/end. At 1024 px (2 KB) a 320x480 frame
 * costs 150 blocking transactions and measured 47.8 ms -- but 307 KB over QSPI
 * at 40 MHz x 4 lanes is only ~15 ms of actual bus time, so ~33 ms was pure
 * per-transaction overhead. 8192 px (16 KB) cuts that to ~19 transactions.
 *
 * Set to 1024 (2 KB transfers), NOT 8192, because the cost is bigger than it
 * first appears. The driver allocates two DMA bounce buffers of MAX_PIXELS*2
 * from internal SRAM -- and because our framebuffer lives in PSRAM, spi_master
 * allocates a FURTHER bounce of the same size per transfer from the DMA-capable
 * heap. At 8192 that is a 16 KB allocation on every transfer.
 *
 * Once Doom rendered at 480 wide, its 138 KB I_VideoBuffer left only ~14 KB of
 * DMA-capable heap, the 16 KB request failed, and the driver asserted inside
 * spi_device_polling_end -- a boot loop with no obvious connection to the
 * resolution change that caused it.
 *
 * Chunk size is worth ~2 ms at most (the real bottleneck was the per-pixel byte
 * swap, not the transaction count), so this is cheap insurance.
 */
