...
/* tdoom: default to HIGH detail (0).
 *
 * detailLevel 1 halves the horizontal resolution of the 3D VIEW ONLY --
 * R_SetViewSize swaps in R_DrawColumnLow/R_DrawSpanLow, which draw each column
 * two pixels wide. The status bar, menus and HUD are unaffected, being drawn as
 * scaled patches.
 *
 * High detail renders all 480 columns rather than 240 doubled ones, which is
 * the whole point of rendering at the panel's native width. It costs little
 * here: measurements showed the frame rate is bounded by the QSPI flush and the
 * blit more than by the renderer, so the extra columns largely consume idle
 * time that was being wasted anyway.
 *
 * Still togglable at runtime from Options -> Graphic Detail; the on-screen
 * readout shows H or L beside the frame rate. */
int			detailLevel = 0;
int			screenblocks = 10;
