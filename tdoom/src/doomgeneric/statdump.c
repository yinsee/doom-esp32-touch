...
/* tdoom: 6.4 KB moved to PSRAM. Only populated when -statdump is passed, which
 * tdoom never does, so this is dead weight in internal RAM. */
TD_LAZY_ARRAY(wbstartstruct_t, captured_stats, MAX_CAPTURES)
#define captured_stats (captured_stats_td_get())
