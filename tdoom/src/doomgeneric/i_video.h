...
/* Doom's native resolution, and the only one that works.
 *
 * 480x320 native was tried and reverted. It rendered noticeably sharper at
 * identical frame rate (29.0 vs 28.6 fps -- we are flush-bound either way, so
 * the extra render cost fitted inside existing idle time), but the engine's
 * fixed-size graphics assume 320x200 in more places than are worth chasing:
 * the status bar, menus and intermission screens are 320-wide WAD patches, the
 * screen blinked, and a new game would not start. Making it right needs
 * Crispy-Doom-style widescreen support, not a resolution change.
 *
 * port_video.c upscales this to the panel's 480x320 instead. */
/* tdoom: native panel width. Single source of truth in td_res.h -- do not
 * hardcode these, they must match doomgeneric.h and the sketch's framebuffer
 * reservation or the heap layout breaks in ways that produce no error.
 *
 * Safe to widen because the parts that care are parameterised: ST_WIDTH is
 * SCREENWIDTH, ST_Y is SCREENHEIGHT-ST_HEIGHT, V_Init allocates nothing, and
 * V_DrawPatch's RANGECHECK only fires when a patch EXCEEDS the screen -- so a
 * wider screen is safe where a narrower one would crash.
 *
 * Cosmetic consequence: status bar, menu and intermission art are 320-wide
 * patches drawn at x=0, so they sit left-aligned rather than centred. */
#include "../port/td_res.h"

#define SCREENWIDTH  TD_DOOM_W
#define SCREENHEIGHT TD_DOOM_H
