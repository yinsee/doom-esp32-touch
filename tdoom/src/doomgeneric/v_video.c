...
/* tdoom: is this draw going to the full screen, and does it need scaling?
 *
 * Only full-screen draws are scaled. Draws into the small backing buffers
 * (st_backing_screen, background_buffer) are in those buffers' own coordinate
 * space and must be left alone. */
static boolean V_ScaleActive(void)
{
    extern byte *I_VideoBuffer;

    return (dest_screen == I_VideoBuffer)
        && (SCREENWIDTH != V_BASEW || SCREENHEIGHT != V_BASEH);
}

/* Design-space -> screen-space edge tables.
 *
 * Built once. The first version divided inline, twice per SOURCE PIXEL, and
 * integer division is slow on Xtensa -- a full-screen menu patch pushed render
 * time from 18ms to 28ms. Table lookups make the inner loop division-free. */
static short v_sx[V_BASEW + 1];
static short v_sy[V_BASEH + 1];
static boolean v_tables_built;

static void V_BuildScaleTables(void)
{
    int i;

    for (i = 0; i <= V_BASEW; i++)
    {
        v_sx[i] = (short)((i * SCREENWIDTH) / V_BASEW);
    }
    for (i = 0; i <= V_BASEH; i++)
    {
        v_sy[i] = (short)((i * SCREENHEIGHT) / V_BASEH);
    }
    v_tables_built = true;
}

/* tdoom: draw a patch scaled from the 320x200 design space to the real screen.
 *
 * Walks the SOURCE (patches are column/post structures with transparency, so
 * iterating the destination would mean a post lookup per pixel) and paints one
 * rectangle per source pixel. Nearest-neighbour: Doom's 2D art is chunky
 * already and filtering it would look worse, not better. */
static void V_DrawPatchScaled(int x, int y, patch_t *patch, boolean flipped)
{
    int col, w;
    ...
    for (col = 0; col < w; col++)
    {
        ...
        while (column->topdelta != 0xff)
        {
            ...
            for (i = 0; i < column->length; i++)
            {
                ...
                for (yy = dy0; yy < dy1; yy++)
                {
                    byte *d = dest_screen + yy * SCREENWIDTH + dx0;

                    for (xx = dx0; xx < dx1; xx++)
                    {
                        *d++ = c;
                    }
                }
...
{ 
    ...
    /* tdoom: scale 320x200-space art up to the real screen. */
    if (V_ScaleActive())
    {
        V_DrawPatchScaled(x, y, patch, false);
        return;
    }
...
void V_DrawPatchFlipped(int x, int y, patch_t *patch)
{
    ...
    if (V_ScaleActive())
    {
        V_DrawPatchScaled(x, y, patch, true);
        return;
    }
