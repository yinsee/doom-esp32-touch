...
void I_InitGraphics (void)
{
	...
	/* tdoom: the renderer writes every pixel of this buffer every frame, so it
	 * must not live in PSRAM. 320*200 = 64 KB out of internal SRAM. */
	I_VideoBuffer = (byte*)DG_AllocScreenBuffer (SCREENWIDTH * SCREENHEIGHT);  // For DOOM to draw on
...
{
	DG_FreeScreenBuffer (I_VideoBuffer);
}
...
{
    /* tdoom: upstream copies I_VideoBuffer into DG_ScreenBuffer here, scaling
     * and converting as it goes. Under CMAP256 at fb_scaling 1 that is a
     * byte-for-byte memcpy producing an identical buffer -- 64KB of internal
     * SRAM we do not have, plus a 64KB copy every frame.
     *
     * port_video.c reads I_VideoBuffer directly instead, so there is nothing
     * to do here but present the frame. */
    DG_DrawFrame();
}
