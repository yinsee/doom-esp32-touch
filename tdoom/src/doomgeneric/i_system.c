...
/* tdoom: 4 MiB of the 8 MiB PSRAM. The rest holds the two 320x480x16bpp
 * canvases (~614 KB) plus IDF/Arduino overhead. */
...
static byte *AutoAllocMemory(int *size, int default_ram, int min_ram)
{
    ...
    while (zonemem == NULL)
    {
        ...
        *size = default_ram * 1024 * 1024;
        ...
        /* tdoom: plain malloc() serves internal SRAM, which cannot hold a
         * multi-megabyte zone. Force the allocation into PSRAM. */
        zonemem = (byte *)DG_AllocZone(*size);
