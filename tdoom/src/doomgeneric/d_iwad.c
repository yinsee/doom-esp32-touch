...
char *D_FindWADByName(char *name)
{
    ...
    /* tdoom: there is no filesystem holding the WAD on this device -- it lives
     * in a raw flash partition that port_wad.c memory-maps (see W_OpenFile).
     * Searching directories for it is meaningless and M_FileExists() would
     * always fail, so hand the name straight back. The name still matters:
     * D_IdentifyIWADByName() uses it to pick the mission/gamemode. */
    return name;
