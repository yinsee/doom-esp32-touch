...
{ 
    ...
    /* tdoom: aim assist.
     *
     * There is no room on a 3.5" touchscreen for precise aiming, so the game
     * presses fire and use itself when the engine says they would do something.
     * Holding BT_USE is safe: P_PlayerThink only acts on the press edge via
     * player->usedown, so a door opens once rather than repeatedly.
     *
     * Placed BEFORE the special-button block on purpose -- those assign
     * cmd->buttons outright and must win. */
	...
	if (TD_AimAssist (&players[consoleplayer], &assistturn, &lined_up))
	{
	    /* Only steer when the player is not turning themselves. Manual
	     * input always wins -- an assist that fights the thumb feels
	     * broken. Turn roughly toward a target, let go, and the assist
	     * finishes the job. */
	    if (cmd->angleturn == 0)
	    {
		cmd->angleturn = (short)assistturn;
	    }

	    /* Shoot only once actually on target; firing mid-swing misses. */
	    if (lined_up)
	    {
		cmd->buttons |= BT_ATTACK;
	    }
	...
	if (TD_UsableInFront (&players[consoleplayer]))
	    cmd->buttons |= BT_USE;
    }
